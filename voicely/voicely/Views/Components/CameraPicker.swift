...
struct CameraPicker: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIImagePickerController {
        // Check camera permission before creating the picker
    ...
    class Coordinator: NSObject, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        ...
        func imagePickerController(_ picker: UIImagePickerController, didFailWithError error: Error) {
            ...
            if let nsError = error as NSError? {
                if nsError.domain == "UIImagePickerControllerErrorDomain" && nsError.code == 1 {
                    // Camera permission denied
                    DispatchQueue.main.async {
                        self.parent.showPermissionAlert = true
                    }
                } else {
                    // Other error
                    DispatchQueue.main.async {
                        self.parent.alertMessage = "Camera error: \(error.localizedDescription)"
                        self.parent.showAlert = true
                    }
                }
            }
